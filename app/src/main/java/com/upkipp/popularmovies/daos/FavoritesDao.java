/*
 * Copyright (C) 2019 Aayat Mimiko
 */

package com.upkipp.popularmovies.daos;

import android.arch.lifecycle.LiveData;
import android.arch.persistence.room.Dao;
import android.arch.persistence.room.Delete;
import android.arch.persistence.room.Insert;
import android.arch.persistence.room.OnConflictStrategy;
import android.arch.persistence.room.Query;
import android.arch.persistence.room.Update;

import com.upkipp.popularmovies.models.MovieData;

import java.util.List;

@Dao
public interface FavoritesDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void addFavorite(MovieData movieData);

    @Query("SELECT * FROM favorites ORDER BY release_date")
    LiveData<List<MovieData>> loadFavorites();

    @Update(onConflict = OnConflictStrategy.REPLACE)
    void updateFavorite(MovieData movieData);

    @Delete
    void deleteFavorite(MovieData movieData);

    @Query("SELECT * FROM favorites WHERE id = :movieId")
    LiveData<MovieData> getMovieById(String movieId);
}
